...
impl OpenRequest {
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("neura-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("neura-dock-action://") {
                this.kind = Some(OpenRequestKind::DockMenuAction {
                    index: action_index.parse()?,
                });
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("neura://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("neura://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("neura://extension/") {
                this.kind = Some(OpenRequestKind::Extension {
                    extension_id: extension_id.to_string(),
                });
            } else if url == "neura://agent" {
                this.kind = Some(OpenRequestKind::AgentPanel);
            } else if let Some(schema_path) = url.strip_prefix("neura://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if let Some(setting_path) = url.strip_prefix("neura://settings/") {
    ...
    fn parse_request_path(&mut self, request_path: &str) -> Result<()> {
        ...
        anyhow::bail!("invalid neura url: {request_path}")
    }
}
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("neura-{}.sock", *RELEASE_CHANNEL_NAME));
...
mod tests {
    use super::*;
    use crate::neura::{open_listener::open_local_workspace, tests::init_test};
    use cli::{
        CliResponse,
        ipc::{self},
    };
    ...
    async fn test_open_workspace_with_directory(cx: &mut TestAppContext) {
        ...
        // Now open a file inside that workspace, but tell Neura to open a new window
        open_workspace_file(
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Neura to add it to the current workspace
        ...
        // but tell Neura to NOT add it to the current workspace
