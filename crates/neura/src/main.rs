...
use crate::neura::{OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "neura failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "neura failed to open a window: {e:?}. See https://neura.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.neura.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("neura failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://neura.dev/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
pub fn main() {
    ...
    // `neura --askpass` Makes neura operate in nc/netcat mode for use with askpass
    ...
    // `neura --crash-handler` Makes neura operate in minidump crash handler mode
    if let Some(socket) = &args.crash_handler {
        crashes::crash_server(socket.as_path());
        return;
    }

    // `neura --nc` Makes neura operate in nc/netcat mode for use with MCP
    ...
    // `neura --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    if args.system_specs {
        ...
        println!("neura System Specs (from CLI):\n{}", system_specs);
        return;
    }

    log::info!(
        "========== starting neura version {}, sha {} ==========",
        app_version,
        app_commit_sha
            .as_ref()
            .map(|sha| sha.short())
        ...
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::neura::listen_for_cli_connections(open_listener.clone()).is_err()
        }

        #[cfg(target_os = "windows")]
        {
            !crate::neura::windows_only_instance::handle_single_instance(
                open_listener.clone(),
                &args,
            )
        }
        ...
        #[cfg(target_os = "macos")]
        {
            use neura::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("neura is already running");
        return;
    }

    let git_hosting_provider_registry = Arc::new(GitHostingProviderRegistry::new());
    let git_binary_path =
        if cfg!(target_os = "macos") && option_env!("NEURA_BUNDLE").as_deref() == Some("true") {
    ...
    app.run(move |cx| {
        ...
        crate::neura::component_preview::init(app_state.clone(), cx);
    ...
    if let Some(kind) = request.kind {
        match kind {
            ...
            OpenRequestKind::Setting { setting_path } => {
                // neura://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // neura://settings/languages/Rust/tab_size  - SUPPORT
...
#[derive(Parser, Debug)]
#[command(name = "neura", disable_version_flag = true)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `neura://` scheme, or relative to <https://kria.in/neura>.
    ...
    /// On macOS, the default is `~/Library/Application Support/Neura`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/Neura`.
    /// On Windows, the default is `%LOCALAPPDATA%\Neura`.
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,

    /// The username and WSL distribution to use when opening paths. If not specified,
    /// Neura will attempt to open the paths directly.
    ///
    ...
    /// Instructs neura to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    dev_server_token: Option<String>,

    /// Prints system specs. Useful for submitting issues on GitHub when encountering a bug
    /// that prevents neura from starting, so you can't run `neura: copy system specs to clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for the MCP Server, to remove the need for netcat as a dependency,
    /// by having neura act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
    nc: Option<String>,

    /// Used for recording minidumps on crashes by having neura run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run neura in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
