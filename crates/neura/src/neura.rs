...
actions!(
    dev,
    [
        /// Stores last 30s of audio from neura staff using the experimental rodio
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://neura.dev/docs/linux
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://neura.dev/docs/linux#could-not-start-inotify");
                    cx.quit();
                })
                .ok();
            }
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://neura.dev/docs/windows
            "#},
            e
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://neura.dev/docs/windows");
                    cx.quit()
                })
                .ok();
            }
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("ZED_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            (
                "DirectX",
                "https://neura.dev/docs/windows",
                "https://neura.dev/docs/windows",
            )
        } else {
            (
                "Vulkan",
                "https://neura.dev/docs/linux",
                "https://neura.dev/docs/linux#neura-fails-to-open-windows",
            )
        };
        let message = format!(
            db::indoc! {r#"
            Neura uses {} for rendering and requires a compatible GPU.
    ...
    workspace
        ...
        .register_action(|_, _: &install_cli::RegisterZedScheme, window, cx| {
            ...
            .detach_and_prompt_err(
                "Error registering neura:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
...
fn open_telemetry_log_file(
    ...
    workspace.with_local_workspace(window, cx, move |workspace, window, cx| {
        ...
        cx.spawn_in(window, async move |workspace, cx| {
            ...
            let header = concat!(
                "// Neura collects anonymous usage data to help us understand how people are using the app.\n",
...
mod tests {
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .neura/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.neura/settings.json"))
            .await
            .unwrap();
        ...
        // 3. Add .neura to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions =
                    Some(vec![".neura".to_string()]);
            });
        });

        eprintln!("Added .neura to file_scan_exclusions in settings");
        ...
        // If .neura is not excluded, the test will fail here
        assert!(
            !has_zed_entry,
            "Test precondition failed: .neura directory should be excluded but was found in worktree"
        );
